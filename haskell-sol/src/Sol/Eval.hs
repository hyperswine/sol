...
evalExpr env (EList es) =
  SList <$> mapM (evalExpr env) es
...
apply _ fn args =
  solError ("cannot apply " ++ showVal fn ++ " to " ++ show (length args) ++ " arguments")
...
applyKey _   (SDict m) (KStr k) =
  case Map.lookup k m of
    Just v  -> return v
    Nothing -> solError ("key not found: " ++ show k ++
                         " in {" ++ intercalate ", " (map show (Map.keys m)) ++ "}")
...
bContains (SStr s) (SStr sub) = return (SBool (sub `isInfixOf` s))
  where isInfixOf needle haystack = any (needle `isPrefixOf`) (tails haystack)
        tails [] = [[]]
...
splitOn sep s  = go s
  where
    ...
    go str
      ...
      | otherwise            = let (w:ws) = go (tail str)
                               in (head str : w) : ws
...
bRm _ [SStr path] = do
  ...
  if isFile then removeFile path
  else if isDir then removeDirectoryRecursive path
...
bSha256 _ [SStr s] = do
  (ec, out, _) <- readProcessWithExitCode "sh"
    ["-c", "printf '%s' " ++ show s ++ " | shasum -a 256"] ""
  case ec of
    ExitSuccess -> return (SStr (takeWhile (/= ' ') (trim out)))
    _           -> do
      (ec2, out2, _) <- readProcessWithExitCode "sh"
...
parseJsonNum s =
  ...
  in if null numStr then Nothing
...
jsonStringify (SDict m) = "{" ++ intercalate "," (map pair (Map.toList m)) ++ "}"
  where pair (k, v) = show k ++ ":" ++ jsonStringify v
