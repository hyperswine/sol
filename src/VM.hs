...
schemeCall env name args = case (name, args) of
  ...
  where
    viaJit scheme f xs macc fallback =
      case (vmJit env, unappliedFn f, toInts xs, macc) of
        (Just jc, Just g, Just is, _)
          | length is >= jitThreshold, accOK macc ->
              compileScheme jc (vmCore env) scheme g >>= \case
                Nothing -> fallback -- not jittable: interpreter's job
                Just addr -> withFuelCell env $ \pfuel ->
                  if scheme == "foldl"
                    then let Just (VInt a0) = macc in VInt . fromIntegral <$> runFold addr pfuel (fromIntegral a0) is
                    else do
                      (_, out) <- runMapFilter addr pfuel is
                      pure (fromInts out)
        _ -> fallback
    accOK (Just (VInt _)) = True
    accOK Nothing = True
    accOK _ = False
...
mkHal :: Int -> Int -> IORef TxState -> IORef Int -> M.Map Name (Int, [Value] -> IO Value)
mkHal pathT handleT tx preempts =
  M.fromList
    [ ("print", (1, \[v] -> putStrLn (render v) >> hFlush stdout >> pure vUnit)),
      ...
      ("!", (2, indexH)),
      ("sleepMs", (1, sleepH)),
      ("input", (1, \_ -> VStr <$> hGetContents' stdin)),
      ("fuelPreempts", (1, \[_] -> VInt . fromIntegral <$> readIORef preempts)),
      ...
      ("writeAll", (2, writeH)),
      ("close", (1, closeH)),
      -- path-level transactional filesystem ops (strings or @paths)
      ("rm", (1, \[v] -> withP v (\p -> txRm tx p >> pure vUnit))),
      ("rmdir", (1, \[v] -> withP v (\p -> txRmdir tx p >> pure vUnit))),
      ("mkdirp", (1, \[v] -> withP v (\p -> txMkdirp tx p >> pure vUnit))),
      ("ls", (1, \[v] -> withP v (\p -> strList <$> txLs tx p))),
      ("exists", (1, \[v] -> withP v (\p -> vBool <$> txExists tx p))),
      ("isDir", (1, \[v] -> withP v (\p -> vBool <$> txIsDir tx p))),
      ("stat", (1, \[v] -> withP v (\p -> do
          (e, sz, mt) <- txStat tx p
          pure (VData 5 0 [vBool e, VInt (fromIntegral sz), VInt (fromIntegral mt)])))),
      -- external commands: sh = immediate query, shq = deferred effect
      ("sh", (1, \[v] -> withS v (\c -> do
          (code, out) <- txSh c
          pure (VData 4 0 [VInt (fromIntegral code), VStr out])))),
      ("shq", (1, \[v] -> withS v (\c -> txShq tx c >> pure vUnit)))
    ]
  where
    ...
    sleepH [VInt ms] = threadDelay (fromIntegral ms * 1000) >> pure vUnit
    sleepH _ = vmPanic "sleepMs: bad args"
...
vecScheme env scheme f macc r = do
  ...
  case (vmJit env, jitCallable env scheme f) of
    (Just _, Just (g, ex)) | getEnvDebug -> putStrLn ("[jit-debug] " ++ scheme ++ " f=" ++ g ++ " extras=" ++ show (length ex) ++ " n=" ++ show n ++ " layout=" ++ show (layoutInfo st))
    (Just _, Nothing) | getEnvDebug -> putStrLn ("[jit-debug] " ++ scheme ++ " fn not JIT-callable: " ++ render f)
    _ -> pure ()
  jitted <- case (vmJit env, jitCallable env scheme f, layoutInfo st, macc) of
    (Just jc, Just (g, extras), Just (scalar, intCols, sig), _)
      | n >= jitThreshold, accInt macc ->
          compileVecScheme jc (vmCore env) scheme g scalar intCols sig (length extras) >>= \case
            Nothing -> pure Nothing
            Just addr -> fmap Just $ withColPtrs st $ \cols -> withFuelCell env $ \pfuel ->
              case scheme of
                "vecfold" -> do
                  let Just (VInt a0) = macc
                  VInt . fromIntegral <$> runVecFold addr pfuel extras cols n (fromIntegral a0)
                "vecmap" -> do
                  (_, out) <- runVecMapFilter addr pfuel extras cols n
                  vecFromInts out
                _ -> do
                  (_, idxs) <- runVecMapFilter addr pfuel extras cols n
